...
def central_agent(net_params_queues, exp_queues, ready_queue):
...
def agent(agent_id, all_cooked_time, all_cooked_bw, net_params_queue, exp_queue, ready_queue, io_lock):
...
def main():
    ...
    coordinator = mp.Process(target=central_agent,
                             args=(net_params_queues, exp_queues, ready_queue))
    coordinator.start()
    ...
    for i in xrange(NUM_AGENTS):
        agents.append(mp.Process(target=agent,
                                 args=(i, all_cooked_time, all_cooked_bw,
                                       net_params_queues[i],
                                       exp_queues[i], ready_queue, io_lock)))
    for i in xrange(NUM_AGENTS):
        agents[i].start()
